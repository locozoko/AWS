...
locals {
  rte1            = join(", ", var.cc-vm1-rte-list)
  rte2            = join(", ", var.cc-vm2-rte-list)
  environs = <<ENVIRONS
{
	"INSTANCES": "${var.cc-vm1-id},${var.cc-vm2-id}",
	"${var.cc-vm1-id}": {
		"RouteTables": "${local.rte1}",
		"HttpProbePort": "${var.http-probe-port}"
	},
	"${var.cc-vm2-id}": {
...
resource "aws_security_group" "lambda-sg" {
  name        = "${var.name-prefix}-port-probe-lambda-sg-${var.resource-tag}"
  description = "Allow HTTP GET access to the specified port on CC"
  vpc_id      = var.vpc

  egress {
    from_port   = var.http-probe-port
    to_port     = var.http-probe-port
    protocol    = "tcp"
...
resource "aws_lambda_function" "cc_route_updater_lambda" {
  filename         = "${path.module}/${var.route-updater-filename}"
  function_name    = "${var.vpc}_cc_route_updater_fn"
  role             = aws_iam_role.iam_for_cc_lambda.arn
  handler          = var.route-updater-handler
  source_code_hash = filebase64sha256("${path.module}/${var.route-updater-filename}")
  runtime          = var.route-updater-runtime
  ...
  vpc_config {
    ...
    subnet_ids         = [var.cc-vm1-snid, var.cc-vm2-snid]
